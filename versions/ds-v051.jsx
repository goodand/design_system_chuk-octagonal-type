...
function Masthead() {
  return (
    <div className="masthead">
      ...
      <div className="meta">v0.5.1 · 2026 · Logo × Body Type</div>
    </div>
  );
}
...
function Hero() {
  return (
    <div className="hero">
      <div>
        <div className="eyebrow">Design System v0.5.1 — Release Notes</div>
...
function Palette() {
  const swatches = [
    ...
    { n: 'accent',      c: '#d9531e', fg: '#faf9f5', usage: 'vertex only · 5%' },
  ...
  return (
    <section>
      <div className="section-head">
        ...
        <h2>Palette</h2>
        <div className="caption">8 CORE tokens · design.md</div>
      </div>
      <div className="palette palette-8">
        {swatches.map(s => (
          <div key={s.n} className="sw" style={{
            ...
            <div className="swu" style={{fontSize: 9, letterSpacing: '0.04em', opacity: 0.7, marginTop: 4}}>{s.usage}</div>
            <div className="swc">{s.c.toUpperCase()}</div>
          </div>
        ))}
      </div>
      <div className="note">
        MD3 surface/role 슬롯은 <b>design-extension.md</b>의 <span className="mono">colors_pending_review</span>에서 추적. 사용자 승인 전에는 적용하지 않음.
      </div>
    </section>
  );
}

// § 08 · Type Scale — Pretendard 정본에 대한 시각적 근거
function TypeScale() {
  const styles = [
    ...
    { token: 'eyebrow',          size: '10 / 500 / 1.0',   tracking: '0.24em',   sample: 'DESIGN SYSTEM v0.5.1',                 usage: 'rv tags · cap text · meta',                kind: 'mono', upper: true },
  ...
  return (
    <section>
      <div className="section-head">
        ...
        <h2>Type Scale</h2>
        <div className="caption">Pretendard 정본 · 9 styles</div>
      </div>
      ...
      <div className="note">
        본문은 <b>Pretendard Variable</b> CDN(한글 + 라틴 단일 스택). mono는 시스템 스택(<span className="mono">ui-monospace</span> → SF Mono → Menlo). Harmony Test 8/8 PASS는 이 조합 기준으로 실측됨.
      </div>
    </section>
  );
}
...
function Footer() {
  return (
    <footer>
      <span>축 · <b>Design System v0.5.1</b></span>
